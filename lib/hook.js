...
function resolveConfig(pkgJsonPath) {
  ...
  return {
    ...
    splashMinDuration: Number.isInteger(ppConfig.splashMinDuration) && ppConfig.splashMinDuration > 0 ? ppConfig.splashMinDuration : 0,
    compression: ["zstd", "gzip", "xz"].includes(ppConfig.compression) ? ppConfig.compression : "zstd",
    level: ppConfig.level || 0,
    lenientLockDetect: ppConfig.lenientLockDetect === true,
  };
}
...
async function runHook(buildResult, { pkgJsonPath, packagerDir, exec, execGo, generateSysoFn } = {}) {
  ...
  const { version, productName, exeName, goArch, splashPath, splashMinDuration, compression, level, lenientLockDetect } = resolveConfig(resolvedPkgJson);
  ...
  try {
    ...
    if (splashMinDuration > 0) packArgs.push("-splash-min-duration", String(splashMinDuration));
    if (lenientLockDetect) packArgs.push("-lenient-lock-detect");
